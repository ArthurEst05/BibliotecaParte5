...
public class Main2 {
    ...
    public static void main(String[] args) {
      ...
      Livro lv1 = new Livro("Batman","Lonardo Da vince");
      Livro lv2 = new Livro("A volta ", "Jorge Ferdinand");
      Livro lv3 = new Livro("Futebol", "Globinho");
      livros.add(lv1);
      livros.add(lv2);
      livros.add(lv3);
      saveLivrosToFile(LIVROS_FILE, livros);

      
    }
    
    private static void saveUsuariosToFile(String filename, ArrayList<Pessoa> list) {
    ...
    private static ArrayList<Pessoa> loadUsuariosFromFile(String filename) {
...
private static void saveLivrosToFile(String filename, ArrayList<Livro> livros) {
    try (FileWriter writer = new FileWriter(filename, false)) {
...
private static void saveReservasToFile(String filename, ArrayList<Reserva> reservas) {
    try (FileWriter writer = new FileWriter(filename, false)) {
...
private static void saveEmprestimosToFile(String filename, ArrayList<Emprestimos> emprestimos) {
    try (FileWriter writer = new FileWriter(filename, false)) {
        for (Emprestimos emprestimo : emprestimos) {
            writer.write(emprestimo.getPessoa().getNome() + ";" + emprestimo.getLivros().getTitulo() + ";" +
                    emprestimo.getDataDoEmprestimo().toString() + ";" + emprestimo.isDevolvido() + "\n");
        }
        writer.flush();
    } catch (IOException e) {
        System.err.println("Erro ao salvar empréstimos no arquivo: " + e.getMessage());
    }
}

private static ArrayList<Livro> loadLivrosFromFile(String filename) {
...
private static Pessoa findUsuarioByName(String nomeUsuario,ArrayList<Pessoa> usuarios) {
...
private static Livro findLivroByTitle(String tituloLivro, ArrayList<Livro> livros) {
    for (Livro livro : livros) {
...
private static ArrayList<Reserva> loadReservasFromFile(String filename) {
...
private static ArrayList<Emprestimos> loadEmprestimosFromFile(String filename) {
...
private static void saveTrabalhosToFile(String filename, ArrayList<Trabalho> trabalhos) {
    try (FileWriter writer = new FileWriter(filename, false)) {
...
private static ArrayList<Trabalho> loadTrabalhosFromFile(String filename) {
